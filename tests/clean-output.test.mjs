...
ok("strips UTF-8 BOM", cleanOutput(BOM + "2\r\n") === "2\n", JSON.stringify(cleanOutput(BOM + "2\r\n")));
...
ok("extractAnswer trims", extractAnswer(BOM + "\n\n  The answer is 42.  \n\n\n") === "The answer is 42.", JSON.stringify(extractAnswer(BOM + "\n\n  The answer is 42.  \n\n\n")));

// 6. Chinese answer survives cleaning.
ok("keeps CJK", cleanOutput(BOM + "玉山\r\n") === "玉山\n", JSON.stringify(cleanOutput(BOM + "玉山\r\n")));

// 7. Mixed ANSI + drip + BOM -> clean multi-line.
{
  const raw = `${BOM}${ESC}[?25l${ESC}[32mPart one.${ESC}[0m\r\nPart two.\r\n${ESC}[?25h`;
