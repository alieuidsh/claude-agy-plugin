...
> **Plugin comunitario no oficial.** Sin relación con Google ni Anthropic. Depende de
> la CLI `agy` instalada localmente y analiza su formato de transcript en disco (no es
> una API pública estable) — consulta [TEST_MATRIX.md](TEST_MATRIX.md) para ver qué se
> ha verificado realmente.

Multiplataforma: **Windows, Linux, macOS** (companion Node puro). Por ahora probado
principalmente en Windows; la detección de rutas cubre diseños habituales de
Windows/macOS/Linux, y puedes definir `AGY_BIN` si `agy` está en otro lugar.
...
2. **La CLI `agy`** (Google Antigravity), instalada y con sesión iniciada una vez.
   - **¿No la tienes?** Ejecuta **`/agy:install`** — el plugin detecta que falta y la
     instala por ti (tras preguntar). O manualmente:
     - Windows: `irm https://antigravity.google/cli/install.ps1 | iex`
     - macOS/Linux: `curl -fsSL https://antigravity.google/cli/install.sh | bash`
   - Si está en otro lugar, define la variable de entorno `AGY_BIN` (ruta completa).
   - Tras instalar, ejecuta `agy` una vez de forma interactiva para iniciar sesión (cuenta Google).
...
El plugin se distribuye por GitHub, así que cualquiera puede obtener las últimas correcciones:

```
/plugin update agy
```

Luego reinicia Claude Code. (Equivalente en CLI: `claude plugin update agy`.) Si un
comando informa de que el formato del transcript de agy no se reconoció, significa
que la CLI `agy` subyacente cambió — ejecuta la actualización anterior para obtener
el plugin corregido.
